...
def felzenszwalb_segmentation(img_2d_one_band,scale=3.0, sigma=0.95, min_size=5):
    # https://scikit-image.org/docs/dev/api/skimage.segmentation.html#skimage.segmentation.felzenszwalb
    out_labels = segmentation.felzenszwalb(img_2d_one_band, scale=scale, sigma=sigma, min_size=min_size)
...
def segment_changes_on_dem_diff(dem_diff_tif, save_dir):
    ...
    # read images
    one_band_img, nodata = raster_tools.read_raster_one_band_np(dem_diff_tif)
    ...
    raster_tools.save_numpy_array_to_rasterfile(out_labels, label_path, dem_diff_tif, nodata=0)
