...
from utility import exec_command_args_list_one_file, outputlogMessage
import numpy as np
...
def zonal_stats_one_polygon(idx, polygon, image_tiles, img_tile_polygons, stats, nodata=None,range=None,
    ...
    if len(image_list) == 1:
        out_image, out_tran,nodata = raster_tools.read_raster_in_polygons_mask(image_list[0], polygon, nodata=nodata,
        ...
        for k_img, image_path in enumerate(image_list):
            ...
            _, _,nodata = raster_tools.read_raster_in_polygons_mask(image_path, polygon,all_touched=all_touched,nodata=nodata,
        ...
        if exec_command_args_list_one_file(mosaic_args_list,save_path) is False:
            raise IOError('error, obtain a mosaic (%s) failed'%save_path)

        # read the raster
        out_image, out_nodata = raster_tools.read_raster_one_band_np(save_path,band=band)
        # remove temporal raster
        tmp_saved_files.append(save_path)
        for item in tmp_saved_files:
            utility.delete_file_or_dir(item)

    else:
        outputlogMessage('warning, cannot find raster for %d (start=0) polygon'%idx)
...
def zonal_stats_multiRasters(in_shp, raster_file_or_files, tile_min_overlap=None, nodata=None, band = 1, stats = None, prefix='',
    ...
    '''
    utility.is_file_exist(in_shp)
    if stats is None:
        outputlogMessage('warning, No input stats, set to ["mean"])')
        stats = ['mean']
    ...
    if isinstance(raster_file_or_files,str):
        utility.is_file_exist(raster_file_or_files)
    ...
    # get image box
    img_tile_boxes =  [raster_tools.get_image_bound_box(tile) for tile in image_tiles]
    ...
    if len(polygons) < 1:
        outputlogMessage('No polygons in %s'%in_shp)
        return False
    ...
    if 'area' in stats_backup:
       dx, dy = raster_tools.get_xres_yres_file(image_tiles[0])
...
def zonal_stats_multiRasters_polygons(polygons, raster_file_or_files, tile_min_overlap=None, nodata=None, band=1, stats=None, prefix='',
    ...
    img_tile_polygons = [vector_gpd.convert_image_bound_to_shapely_polygon(box) for box in img_tile_boxes]
    if len(polygons) < 1:
        outputlogMessage('No input polygons')
        return None
...
def test_zonal_stats_multiRasters():
    ...
    dem_list = utility.get_file_list_by_ext('.tif',dem_file_dir,bsub_folder=False)
    save_shp = os.path.basename(utility.get_name_by_adding_tail(shp, 'multi_raster_stats'))

    utility.copy_shape_file(shp, save_shp)
