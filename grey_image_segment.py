...
from image_segment import felzenszwalb_segmentation
...
def segment_a_patch(idx, patch, patch_count,img_path, org_raster,b_save_patch_label):
    ...
    image_name_no_ext = utility.get_name_no_ext(img_path)
    ...
    if b_save_patch_label is True:
        # save the label
        raster_tools.save_numpy_array_to_rasterfile(out_labels, patch_label_path, img_path, boundary=patch) # it copy nodata, need to unset it later
        return patch, patch_label_path, None, None
    ...
    if org_raster is not None:
        org_img_b1, org_nodata = raster_tools.read_raster_one_band_np(org_raster, boundary=patch)
...
def segment_a_grey_image(img_path, save_dir,process_num, org_raster=None,b_save_patch_label=False):
    ...
    if os.path.isfile(label_path):
        print('%s exist, skip segmentation'%label_path)
        return label_path

    height, width, band_num, date_type = raster_tools.get_height_width_bandnum_dtype(img_path)
    ...
    if org_raster is not None:
        org_height, org_width, org_band_num, org_date_type = raster_tools.get_height_width_bandnum_dtype(org_raster)
    ...
    image_patches = raster_tools.sliding_window(width,height, 1024, 1024,adj_overlay_x=0,adj_overlay_y=0)
    ...
    if object_attributes:
        ...
        utility.save_dict_to_txt_json(attribute_path,object_attributes)

    # save the label
    raster_tools.save_numpy_array_to_rasterfile(save_labes, label_path, img_path) # do not set nodata
    ...
    utility.save_list_to_txt(label_id_range_txt,patch_label_id_range)
