...
def flood_detection_from_SAR_amplitude(sar_image_list, save_dir,dst_nodata=128, src_nodata=None, water_mask_file=None,ptf=False,v=0.1,verbose=False,process_num=1):
    ...
    for idx, grd in enumerate(sar_image_list):
        ...
        img_data, min, max, mean, medium = image_read_pre_process(grd, src_nodata=src_nodata)
        print(datetime.now(),'read and preprocess, size:',img_data.shape,'min, max, mean, medium',min, max, mean, medium)
...
def main(options, args):
    ...
    flood_detection_from_SAR_amplitude(sar_image_list, save_dir, dst_nodata=dst_nodata,src_nodata=src_nodata,
                                       water_mask_file=water_mask, ptf=False, v=0.1,verbose=verbose,process_num=process_num)
