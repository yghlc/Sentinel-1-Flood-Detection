...
from utility import outputlogMessage
...
def fix_invalid_polygons(polygons, buffer_size = 0.000001):
    ...
    if len(invalid_polygon_idx) > 0:
        outputlogMessage('Warning, polygons %s (index start from 1) in are invalid, fix them by the buffer operation '%(str(invalid_polygon_idx)))
...
def find_polygon_intersec_polygons(shp_path):
    ...
    outputlogMessage('Checking duplicated polygons in %s'%shp_path)
    ...
    for idx, poly in enumerate(polygons):
        for kk in range(idx+1,count):
            ...
            if inter.is_empty is False:
                outputlogMessage('warning, %d th polygon has intersection with %d th polygon'%(idx+1, kk+1))
                # break
    outputlogMessage('finished checking of polygons intersect other polygons')
...
def add_attributes_to_shp(shp_path, add_attributes,save_as=None,format='ESRI Shapefile'):
    ...
    for key in add_attributes.keys():
        if key in org_attribute_names:
            outputlogMessage('warning, field name: %s already in table '
...
def read_attribute_values_list(polygon_shp, field_name):
    ...
    if field_name in shapefile.keys():
        ...
        outputlogMessage('Warning: %s not in the shape file, will return None'%field_name)
        return None
...
def read_polygons_attributes_list(polygon_shp, field_nameS, b_fix_invalid_polygon = True):
    ...
    if isinstance(field_nameS,str): # only one field name
        if field_nameS in shapefile.keys():
            ...
            outputlogMessage('Warning: %s not in the shape file, get None' % field_nameS)
...
def remove_polygon_equal(shapefile,field_name, expect_value, b_equal, output):
    ...
    outputlogMessage('remove %d polygons based on %s, remain %d ones saving to %s' %
...
def remove_polygons_not_in_range(shapefile,field_name, min_value, max_value,output):
    ...
    if len(shapefile.geometry.values) < 1:
        outputlogMessage('remove %d polygons based on %s, remain %d ones, no saved files' %
...
def remove_polygons_based_values(shapefile,value_list, threshold, bsmaller,output):
    ...
    if len(shapefile.geometry.values) < 1:
        outputlogMessage('remove %d polygons based on a list of values, remain %d ones, no saved files' %
                               (remove_count, len(shapefile.geometry.values)))
        return False
    else:
        outputlogMessage('remove %d polygons, remain %d ones saving to %s' %
...
def save_shapefile_subset_as(data_poly_indices, org_shp, save_path,format='ESRI Shapefile'):
    ...
    if guess_format != format:
        outputlogMessage('warning, the format (%s) associated with file extension is different with the input one (%s)'%
                               (guess_format,format))
        format = guess_format

    shapefile_sub.to_file(save_path, driver=format)
    outputlogMessage('save subset (%d geometry) of shapefile to %s'%(save_count,save_path))
    ...
    if multiPolygon.geom_type == 'GeometryCollection':
        ...
        for geometry in multiPolygon:
            ...
            if geometry.geom_type == 'Polygon':
                ...
                outputlogMessage("Warning, abandon a %s derived from the %d th polygon "%(geometry.geom_type,idx))
...
def fill_holes_in_polygons_shp(in_shp, out_shp):
    ...
    for idx, row in shapefile.iterrows():
        ...
        if poly.type == 'MultiPolygon':
            ...
            outputlogMessage('Warning, %d geometry is "MultiPolygon", convert it to Polygons and copy attributes' % idx)
...
def build_adjacent_map_of_polygons(polygons_list, process_num = 1):
    ...
    if polygon_count < 2:
        outputlogMessage('error, the count of polygon is less than 2')
        return False
